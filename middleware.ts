...
function getPreferredLanguage(acceptLanguage: string) {
...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith(`/${supportedLanguages[0]}`) || pathname.startsWith(`/${supportedLanguages[1]}`)) {
  ...
  if (cookieLang) {
    ...
    if (supportedLanguages.includes(langLower)) {
      return NextResponse.redirect(new URL(`/${langLower}${pathname}`, request.url))
    }
  }

  // Obtener idioma del navegador
  const acceptLanguage = request.headers.get('Accept-Language') || defaultLanguage
